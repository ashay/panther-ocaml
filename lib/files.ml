 ...
 * terminates. *)
let listen_for_fs_updates (pid : int) (key : Types.raw_string)
    (sync_filepath : string) (tmp_filepath : string)
    (events : Fswatch.Event.t array) : unit =
  (* Number of seconds to sleep in between reads of message queue. *)
  let sleep_count = 1 in
  ...
  | _ ->
      ...
      | true -> (
          ...
          | Ok _ -> ()
          | Error message -> Printf.eprintf "err: %s\n" message )
...
let __monitor_editor (key : Types.raw_string) (sync_filepath : string)
  ...
  | Ok pid -> (
      ...
      | Fswatch.Status.FSW_OK ->
          ...
          let callback_fn = listen_for_fs_updates pid key sync_filepath tmp_filepath in
          let handle = Fswatch.init_session monitor callback_fn in

          (* Tell fswatch to monitor the specific file. *)
          Fswatch.add_path handle tmp_filepath;
          Fswatch.start_monitor handle;
...
let monitor_editor (key : Types.raw_string) (sync_filepath : string)
    (editor : string) (tmp_filepath : string) : unit =
  __monitor_editor key sync_filepath editor tmp_filepath
